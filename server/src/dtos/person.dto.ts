...
const PersonCreateSchema = z
  .object({
    ...
    birthDate: emptyStringToNull(isoDateToDate.nullable())
      .optional()
      .refine((val) => (val ? val <= new Date() : true), { error: 'Birth date cannot be in the future' })
...
const PersonResponseSchema = z
  .object({
    ...
    // use this when using `ZodSerializerDto` on the controllers. Then the codec in `isoDateToDate` in validation.ts will handle the conversion instead.
    // using this now would return a iso datetime string instead of a date string
    // birthDate: isoDateToDate
    //   .describe('Person date of birth')
    //   .nullable(),
