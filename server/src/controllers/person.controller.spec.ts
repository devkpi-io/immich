...
describe(PersonController.name, () => {
  ...
  describe('PUT /people/:id', () => {
    ...
    it('should not accept an invalid birth date (false)', async () => {
      ...
      expect(status).toBe(400);
      expect(body).toEqual(
        errorDto.badRequest(['[birthDate] Invalid input: expected ISO date string (YYYY-MM-DD), received boolean']),
      );
    });
    ...
    it('should not accept an invalid birth date (number)', async () => {
      ...
      expect(status).toBe(400);
      expect(body).toEqual(
        errorDto.badRequest(['[birthDate] Invalid input: expected ISO date string (YYYY-MM-DD), received number']),
      );
    });
